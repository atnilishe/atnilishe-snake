...
app.get('/main_menu', function(req, response) {
    response.sendFile(path.join(__dirname, './client/views/main_menu.html'));
});

app.get('/battle', function(req, response) {
    response.sendFile(path.join(__dirname, './client/views/battle.html'));
});
    ...
    if (req.body.nickname) {
        ...
        http.get(url, (res) => {
            res.on('data', (d) => {
                ...
                req.session.save();
                response.writeHead(302, { 'Location': '/main_menu' });
                response.end();
            });
        }).on('error', (e) => {
            console.error(e);
